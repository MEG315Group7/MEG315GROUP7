#!/usr/bin/env python3
"""
Standalone Python-based Frontend for  Group 7 AD-HTC System
Works without backend server by importing modules directly
"""
...
try:
    ...
    # Fallback classes for when backend modules aren't available
    class BraytonCycle:
        def __init__(self, ambient_temp=288, pressure_ratio=6, max_turbine_temp=1000, 
...
class ADHTCStandaloneFrontend:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Group 7 - AD-HTC Fuel-Enhanced Gas Power Cycle (Standalone)")
    ...
    def setup_ui(self):
        ...
        tk.Button(btn_frame, text="Reset", command=self.reset_simulation,
                 bg=self.colors['warning'], fg='black', width=8).pack(side=tk.LEFT, padx=2)
        ...
        speed_scale = tk.Scale(speed_frame, from_=0.1, to=3.0, resolution=0.1,
                              ...
                              length=120, bg=self.colors['surface'])
        ...
        tk.Button(btn_frame2, text="Calculate Scenario", command=self.calculate_scenario,
                 bg=self.colors['primary'], fg='white', width=15).pack(pady=2)
        
        tk.Button(btn_frame2, text="Custom Calculation", command=self.calculate_custom,
                 bg=self.colors['secondary'], fg='white', width=15).pack(pady=2)
        ...
        for label, key, min_val, max_val in params:
            ...
            tk.Label(frame, text=label, bg=self.colors['surface'], width=18).pack(side=tk.LEFT)
        ...
        self.results_text = tk.Text(results_frame, height=15, width=35,
                                   bg=self.colors['surface'], font=('Courier', 8))
        ...
        # Charts panel
        charts_frame = tk.Frame(main_frame, bg=self.colors['surface'],
                               relief=tk.RAISED, bd=2, height=200)
        charts_frame.pack(side=tk.BOTTOM, fill=tk.X, padx=5, pady=5)
    ...
    def update_charts(self):
        ...
        # Chart 1: Energy Flow
        energy_sources = ['Biogas', 'Hydrochar', 'Waste Heat']
        energy_values = [
            results['biogas_production'] * 6,  # kWh/day
            results['hydrochar_production'] * 25 / 3.6,  # Convert MJ to kWh
            results['total_power_output'] * 0.3  # Estimated waste heat
        ]
        
        ax1.bar(energy_sources, energy_values, color=['#228B22', '#8B4513', '#FF4500'])
        ax1.set_title('Energy Sources (kWh/day)')
        ax1.set_ylabel('Energy (kWh/day)')
        
        # Chart 2: System Performance
        metrics = ['Power Output\n(kW)', 'Efficiency\n(%)', 'Biogas\n(m³/day)']
        values = [
            results['total_power_output'],
            results['overall_efficiency'],
            results['biogas_production']
        ]
        
        bars = ax2.bar(metrics, values, color=['#4682B4', '#32CD32', '#228B22'])
        ...
        # Chart 3: Process Flow
        processes = ['AD', 'Combustion', 'Turbine', 'HTC']
        efficiencies = [85, 95, 88, 35]  # Typical efficiencies
        
        ax3.bar(processes, efficiencies, color=['#90EE90', '#FF6B35', '#4682B4', '#8B4513'])
        ax3.set_title('Process Efficiencies (%)')
